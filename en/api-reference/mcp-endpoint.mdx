...
<Note>
  An alternative transport path `/mcp/mcp` is also supported and is functionally identical.
</Note>
...
<Steps>
  ...
  <Step title="Parameter substitution">
    Replace `{parameter}` placeholders with provided argument values. Values are single-quote-escaped and wrapped in quotes to prevent SQL injection.
  </Step>
  ...
  <Step title="CTE wrapping">
    Wrap the Agent View's SQL as a CTE, so the tool query can reference the view by name:
    ```sql
    WITH "viewName" AS (
      -- Agent View SQL
    )
    -- Tool SQL referencing viewName
    ```
  </Step>
  ...
  <Step title="Execute query">
    Connect to the datasource (decrypting credentials) and run the combined CTE + tool query.
  </Step>
  <Step title="Apply PII masking">
    If masking is enabled, apply JSON-based PII masking rules to the result set. Each rule maps a column to a predefined masking type (e.g., `cpf`, `email`, `phone`, `name`).
  </Step>
