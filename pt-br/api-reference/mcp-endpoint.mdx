...
<Note>
  Um caminho de transporte alternativo `/mcp/mcp` também é suportado e é funcionalmente idêntico.
</Note>
...
<Steps>
  ...
  <Step title="Substituição de parâmetros">
    Substitui os placeholders `{parameter}` pelos valores dos argumentos fornecidos. Os valores são escapados com aspas simples para prevenir injeção SQL.
  </Step>
  ...
  <Step title="Encapsulamento via CTE">
    Encapsula o SQL da Agent View como uma CTE, para que a query da tool possa referenciar a view pelo nome:
    ```sql
    WITH "viewName" AS (
      -- SQL da Agent View
    )
    -- SQL da Tool referenciando viewName
    ```
  </Step>
  ...
  <Step title="Execução da query">
    Conecta à datasource (descriptografando as credenciais) e executa a query combinada CTE + tool.
  </Step>
  <Step title="Aplicação de mascaramento de PII">
    Se o mascaramento está habilitado, aplica as regras de mascaramento de PII baseadas em JSON ao conjunto de resultados. Cada regra mapeia uma coluna para um tipo de mascaramento predefinido (ex.: `cpf`, `email`, `phone`, `name`).
  </Step>
